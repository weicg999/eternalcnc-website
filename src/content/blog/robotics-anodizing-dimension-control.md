...
**Anodizing is the only step in the chain where the part comes back measurably a different size than it left.**

Everything else — milling, turning, drilling — removes material in a place you chose, at a moment you chose, on a machine you can see. Anodizing grows a layer on every bare surface at once, including the surfaces you just spent four setups getting to H7.
...
An anodic coating isn't paint sitting on top of the metal. It's aluminum that has been converted into aluminum oxide, and the oxide takes up more room than the metal it came from. The practical result, and the number you need:
...
Anodize a whole robot joint "all over" and these are the failures we see, roughly in order of frequency:
...
4. **Grounding and electrical contact faces.** Anodic coating is an electrical insulator — that's half of why it resists corrosion. A robot assembled from fully anodized mating faces has no reliable ground path, and then you chase EMC problems that aren't really EMC problems.
5. **Fatigue-critical surfaces under hardcoat.** The coating is hard and brittle. Under cyclic load it cracks, and those cracks start cracks in the aluminum underneath. Hard anodizing is widely reported to reduce fatigue strength of the substrate, alloy-dependent and significantly so on the high-strength grades. Robot links and joint arms are cyclically loaded by definition. Think before you hardcoat a part that flexes.
...
| **Tap oversize before anodizing** | Free in unit cost, needs the right tap (an "anodizing oversize" or high-limit tap) and a one-off trial to size it. Our default on volume work. |
| **Chase the thread after** | Cheap and immediate — but it cuts through the coating into bare aluminum, so you've created the corrosion path you anodized to avoid. |
| **Thread insert after anodizing** | Best wear and corrosion result. Costs an insert per hole and needs the boss wall thick enough. |

The design-level answer, if you can still change the part: **don't anodize the threads at all.** Use clearance holes with nuts, or put the threads in a steel insert or a separate bracket. Every threaded hole you delete from the anodize note is a hole you don't pay to mask, plug, chase or scrap.

## Color: why batch two never matches batch one

Robot parts are visible parts. Black anodized arms and end effectors sit in trade-show photos and on customer floors, and color mismatch is the complaint that arrives after everything else is perfect.

Anodizing is translucent, not opaque, and the dye sits in the pores. So the final color is a product of the alloy underneath as much as the dye:

- **Alloy.** 6061 anodizes clean and predictable. 7075 goes visibly darker and greyer thanks to the zinc. 2xxx (high copper) tends yellow-brown. Cast alloys with high silicon go blotchy grey no matter what you do.
...
- **Material lot.** Two bars of 6061-T6 from different heats, same machine, same tank, same day — still a visible step. If you care, **buy the spare parts' material with the production material**, and anodize them in the same rack.
...
The default in every anodizing shop is "anodize everything bare." That default is what causes the surprises, so write the note as a list of **exclusions**, not inclusions:
...
- **Say where the rack may touch the part.** Parts have to be electrically contacted to be anodized, so there will be contact marks. If you don't say where, the operator chooses, and operators choose the big flat face you were going to look at.
...
**Can you just machine everything to final size and anodize thinner?**
...
**Does anodizing affect the dimensional tolerance on the parts we already made?**

It changes the size, so yes. If a bore was measured at Ø30.010 before anodizing, it will measure about Ø29.998 after a 12 µm Type II. Measure after anodizing for anything that matters, and state on the drawing whether the tolerance applies before or after coating — most drawings don't, which is why this argument happens at goods-in.

**Can you strip and re-anodize a part that came out wrong?**

Usually yes, once. Stripping removes a thin layer of aluminum along with the coating, so a second strip often takes the part out of tolerance. Treat re-work as one shot, not a loop — and don't plan a process that depends on it.

**Is hardcoat always better for robot parts?**

No. Hardcoat is for wear — sliding surfaces, gripper jaws, anything that rubs. It's thicker (so worse for dimensions), darker and less consistent in color, and it costs fatigue life. Most robot structural parts want Type II. The ones that want Type III know why.

## Where we fit

We machine the pre-anodize dimensions, run a masking plan off your drawing, and measure the parts after they come back from the tank — because that's the number your assembly sees. When a callout and a coating spec are in conflict, we'd rather flag it during DFM than ship parts that gauge correctly in our inspection room and bind on your floor.
